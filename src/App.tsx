      ...
      case 'Trang Chủ':
        return (
          <div className="space-y-20 font-sans">
            {/* Hero Section */}
            <div className="relative h-[600px] flex items-center justify-center rounded-3xl overflow-hidden bg-white">
              ...
              <div className="relative text-center text-[#1a3c6e] space-y-4 p-8">
                <EditableText id="hero_title" text={customText.hero_title || 'MD HOME SMART PHỐ HIẾN'} isEditing={isEditing} onSave={handleSaveText} className="text-3xl md:text-5xl font-black leading-normal tracking-wide text-[#1a3c6e]" as="h1" />
                <EditableText id="hero_subtitle" text={customText.hero_subtitle || 'KIẾN TẠO CỘNG ĐỒNG SỐNG HIỆN ĐẠI - VĂN MINH - BỀN VỮNG'} isEditing={isEditing} onSave={handleSaveText} className="text-base md:text-xl uppercase text-white font-extrabold leading-relaxed tracking-wider" as="p" />
                <EditableText id="hero_tagline" text={customText.hero_tagline || 'Không gian sống xanh, tiện ích đồng bộ, kết nối thuận tiện và cơ hội sở hữu nhà ở với chi phí hợp lý.'} isEditing={isEditing} onSave={handleSaveText} className="text-sm md:text-base font-semibold max-w-2xl text-white/90 leading-relaxed" as="p" />
                <div className="pt-8 flex gap-4 justify-center">
                  <button 
                    ...
                    className="bg-[#c9a227] text-white px-10 py-4 rounded-full font-bold text-lg hover:bg-yellow-600 transition cursor-pointer font-sans"
                  >
                    Khám phá dự án
                  </button>
                  <a href={`tel:${MD_CONFIG.hotline1}`} className="bg-white text-[#1a3c6e] px-10 py-4 rounded-full font-bold text-lg hover:bg-gray-100 transition">{MD_CONFIG.hotline1}</a>
                </div>
              </div>
            </div>
                  ...
                  <div className="flex-1 space-y-8">
                     ...
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {[
                           ...
                           },
                           {
                            ...
                            <div key={idx_item} className="bg-[#15305c] p-5 rounded-2xl border border-blue-800/80 hover:border-amber-500/50 transition duration-300 flex items-start gap-4 shadow-sm group">
                     ...
                     <div className="flex flex-col sm:flex-row gap-4 pt-4">
                        <button 
                          ...
                          className="flex-1 bg-white text-[#1a3c6e] p-5 rounded-2xl text-center font-bold text-lg hover:bg-gray-100 transition shadow-md cursor-pointer font-sans"
                        >
                          NHẮN TIN<br/>Zalo {MD_CONFIG.hotline1}
                        </button>
                        <a href={`tel:${MD_CONFIG.hotline1}`} className="flex-1 bg-[#c9a227] text-white p-5 rounded-2xl text-center font-bold text-lg hover:bg-[#b08d20] transition shadow-md">GỌI NGAY<br/>{MD_CONFIG.hotline1}</a>
                     </div>
                  </div>
          ...
          <div className="space-y-8 animate-fade-in font-sans">
            ...
            <div className="bg-white border border-slate-100 rounded-3xl p-5 shadow-sm space-y-4">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-100 pb-3">
                <div className="space-y-1">
                  <h4 className="font-extrabold text-[#1a3c6e] text-md uppercase flex items-center gap-1.5">
                    <span>📊</span> Bảng Data Hotline Đăng Ký (Kiểu Google Sheets)
                  </h4>
                  <p className="text-[11px] text-slate-400 font-sans">Báo cáo hợp nhất từ tất cả các form đăng ký trực tuyến và Trợ Lý AI Chatbot.</p>
                </div>
                <button 
                  onClick={handleExcelDownload}
                  className="bg-emerald-600 hover:bg-emerald-700 text-white font-extrabold text-xs uppercase tracking-wider px-4 py-2.5 rounded-xl transition-all duration-300 flex items-center gap-1.5 shadow-sm cursor-pointer"
                >
  ...
  return (
    <div className="min-h-screen flex flex-col relative">
      ...
      <main className="flex-grow container mx-auto p-4 py-8">
        ...
        <div className="bg-white shadow-lg rounded-2xl p-6 md:p-8 text-[#1a3c6e]">{renderContent(isEditingText)}</div>
      </main>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto transition-opacity duration-300">
          <div className="bg-white rounded-3xl max-w-3xl w-full max-h-[90vh] overflow-y-auto relative p-6 md:p-8 shadow-2xl border border-slate-100 transform transition-transform duration-300 scale-100">
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-4 text-slate-700 leading-relaxed text-sm md:text-base font-sans whitespace-pre-wrap font-medium">
                {selectedNews.content ? (
                  selectedNews.content.split('\n\n').map((paragraph: string, idx: number) => (
                    <p key={idx} className="leading-relaxed">{paragraph}</p>
                  ))
                ) : (
                  <p className="leading-relaxed">{selectedNews.desc}</p>
                )}
              </div>
