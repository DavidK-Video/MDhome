...
import { Save, Image as ImageIcon, Type, LogOut, ChevronDown, ChevronUp, ShieldAlert } from 'lucide-react';

export default function AdminBar({ onClose, setEditText, setEditImages }: {
    onClose: () => void,
    setEditText: (val: boolean) => void,
    setEditImages: (val: boolean) => void
}) {
    const [editImages, setLocalEditImages] = useState(false);
    const [editText, setLocalEditText] = useState(false);
    const [isMinimized, setIsMinimized] = useState(false);

    const toggleEditImages = () => {
        const newVal = !editImages;
        setLocalEditImages(newVal);
        setEditImages(newVal);
    };

    const toggleEditText = () => {
        const newVal = !editText;
        setLocalEditText(newVal);
        setEditText(newVal);
    };
    ...
    return (
        <div className={`fixed bottom-0 left-0 w-full z-[1000] transition-all duration-300 font-sans shadow-[0_-10px_40px_rgba(0,0,0,0.355)] ${isMinimized ? 'translate-y-[calc(100%-40px)]' : 'translate-y-0'}`}>
